...
3. Start crabcode on the host's Tailscale IP or hostname.
4. Open the tailnet URL from your phone browser or attach from another laptop.
...
If binding directly to the Tailscale IP is awkward on your machine, this also works:

```bash
crabcode serve --bind 0.0.0.0:8421 --paircode random
```

Then use the Tailscale IP or MagicDNS name from your clients. Binding to `0.0.0.0` also listens on LAN interfaces, so prefer the specific tailnet IP when you can.
...
| VPS | Remote development from anywhere. | Use key-based SSH, a firewall, and a private network if possible. |
