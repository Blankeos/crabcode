...
| Tools | Tool calls and completed or failed results stream with ACP tool kinds, titles, raw input, full textual output, and bounded preview output. | No normalized locations, diffs, or result images yet. | Preserve structured tool locations, patches, and image content in runtime events. |
