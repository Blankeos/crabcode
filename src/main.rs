...
enum Command {
    ...
    Completion {
        /// Write the script where the shell autoloads it (not stdout)
        #[usage(long)]
        install: bool,
        /// Overwrite a completion file that was not generated by usage
        #[usage(long)]
        force: bool,
        /// Shell: bash, zsh, fish, elvish, nu, powershell. Default: $SHELL
        #[usage(long)]
        shell: Option<String>,
    },
...
fn completion_shell_from_path(shell: Option<&str>) -> usage::complete::Shell {
    match shell.and_then(|shell| shell.rsplit('/').next()) {
        Some("zsh") => usage::complete::Shell::Zsh,
        Some("fish") => usage::complete::Shell::Fish,
        Some("elvish") => usage::complete::Shell::Elvish,
        Some("nu" | "nushell") => usage::complete::Shell::Nu,
        Some("pwsh" | "powershell") => usage::complete::Shell::PowerShell,
        _ => usage::complete::Shell::Bash,
    }
}

fn resolve_completion_shell(explicit: Option<&str>) -> Result<usage::complete::Shell> {
    match explicit {
        Some(name) => usage::complete::Shell::from_name(name).ok_or_else(|| {
            anyhow::anyhow!(
                "unknown shell {name:?}; expected bash, zsh, fish, elvish, nu, or powershell"
            )
        }),
        None => Ok(completion_shell_from_path(
...
const CRABCODE_RC_BEGIN: &str = "# >>> crabcode installer >>>";
const CRABCODE_RC_END: &str = "# <<< crabcode installer <<<";

fn print_completion(shell: usage::complete::Shell) -> Result<()> {
...
fn zsh_installer_block(script: &Path) -> String {
    format!(
        "{CRABCODE_RC_BEGIN}\n[[ -r {} ]] && source {}\n{CRABCODE_RC_END}\n",
        display_home_path(script),
        display_home_path(script)
    )
}
...
fn strip_marked_block(rc: &str) -> String {
    let Some(start) = rc.find(CRABCODE_RC_BEGIN) else {
        return rc.to_string();
    };
    let Some(end_rel) = rc[start..].find(CRABCODE_RC_END) else {
        return rc.to_string();
    };
    let end = start + end_rel + CRABCODE_RC_END.len();
    let mut out = String::new();
...
fn upsert_marked_block(rc: &str, block: &str) -> String {
    ...
    let insert_at = rc.len();
    let mut out = String::new();
    out.push_str(rc[..insert_at].trim_end());
    if !out.is_empty() {
        out.push('\n');
        out.push('\n');
    }
    out.push_str(block);
    out.push('\n');
    let rest = rc[insert_at..].trim_start_matches(['\r', '\n']);
    if !rest.is_empty() {
        out.push('\n');
        out.push_str(rest);
        if rc.ends_with('\n') && !out.ends_with('\n') {
...
fn ensure_zsh_hook(script: &Path) -> Result<PathBuf> {
    let path = resolve_existing_path(&zshrc_path());
    let existing = std::fs::read_to_string(&path).unwrap_or_default();
    let next = upsert_marked_block(&existing, &zsh_installer_block(script));
    if next != existing {
...
fn install_completion(shell: usage::complete::Shell, force: bool) -> Result<()> {
    let env = usage::install::Env::from_process();
    let on_foreign = if force {
        usage::install::OnForeign::Overwrite
    } else {
        usage::install::OnForeign::Refuse
    };
    let installed = Args::install_completion(shell, &env, on_foreign)
        .map_err(|err| anyhow::anyhow!("{err}"))?;

    println!("script {}", display_home_path(&installed.plan.path));

    if shell == usage::complete::Shell::Zsh {
        let rc_path = resolve_existing_path(&zshrc_path());
        let rc = std::fs::read_to_string(&rc_path).unwrap_or_default();
        let aliases = crabcode_aliases_from_rc(&rc);
        let script = std::fs::read_to_string(&installed.plan.path)?;
        let patched = with_zsh_compdef_names(&script, &aliases);
        write_text_atomic(&installed.plan.path, &patched)?;
        let rc = ensure_zsh_hook(&installed.plan.path)?;
        println!("hook   {}", display_home_path(&rc));
        println!("Restart the shell (`exec zsh`) and press Tab.");
        return Ok(());
    }

    match &installed.plan.loading {
        usage::install::Loading::Automatic => {}
        usage::install::Loading::Manual { line, file, .. } => {
            println!("Add this to {file} (once):\n{line}");
        }
        _ => {}
    }
...
fn run_completion(install: bool, force: bool, shell: Option<&str>) -> Result<()> {
...
async fn main() -> Result<()> {
    ...
    match &args.command {
        ...
        Some(Command::Completion {
            ...
            run_completion(*install, *force, shell.as_deref())?;
            return Ok(());
        }
...
mod tests {
    ...
    fn generates_bash_completion() {
        let script = String::from_utf8(generate_completion(completion_shell_from_path(Some(
            "/bin/bash",
        ))))
        .unwrap();
    ...
    fn completion_help_uses_root_help() {
        ...
        assert!(help.contains("Generate or install shell completion"));
    ...
    #[test]
    fn parses_completion_install_flags() {
        let args = parse_args(&["crabcode", "completion", "--install", "--shell", "zsh"]);
        match args.command {
            Some(Command::Completion {
                install,
                force,
    ...
    #[test]
    fn completion_install_plan_puts_zsh_on_fpath() {
        let env = usage::install::Env::from_process().with("HOME", "/tmp/crabcode-test-home");
        let plan = Args::completion_install_plan(usage::complete::Shell::Zsh, &env).expect("plan");
        assert!(
            plan.path.ends_with("zsh/site-functions/_crabcode"),
            "{}",
            plan.path.display()
        );
        match plan.loading {
            usage::install::Loading::Manual { line, file, .. } => {
                assert!(line.contains("fpath"), "{line}");
                assert!(file.contains(".zshrc"), "{file}");
            }
            _ => panic!("expected zsh fpath instruction"),
        }
    }

    #[test]
    fn upsert_zsh_hook_strips_pasted_script_and_is_idempotent() {
        let pasted = "alias cc=crabcode\n#compdef crabcode\n# @generated by usage-argv for `crabcode __complete_word__ --shell zsh`\n_crabcode() {\n    true\n}\ncompdef _crabcode 'crabcode'\n\n# >>> grok installer >>>\nfpath=(~/.grok/completions/zsh $fpath)\n# <<< grok installer <<<\n";
        let script = Path::new("/Users/carlo/.local/share/zsh/site-functions/_crabcode");
        let once = upsert_marked_block(pasted, &zsh_installer_block(script));
        assert!(!once.contains("#compdef crabcode"));
        assert!(once.contains(CRABCODE_RC_BEGIN));
        assert!(once.contains("source ~/.local/share/zsh/site-functions/_crabcode"));
        assert!(!once.contains("compinit"));
        assert!(once.contains("# >>> grok installer >>>"));
        assert!(
            once.find("# >>> grok installer >>>").unwrap() < once.find(CRABCODE_RC_BEGIN).unwrap()
        );
        assert_eq!(
            once,
            upsert_marked_block(&once, &zsh_installer_block(script))
        );
    }

    #[test]
    fn zsh_compdef_includes_cc_alias() {
        let aliases = crabcode_aliases_from_rc("alias cc=\"crabcode\"\nalias lg=lazygit\n");
        assert_eq!(aliases, vec!["cc".to_string()]);
        let generated = Args::completion_script(usage::complete::Shell::Zsh);
        let script = with_zsh_compdef_names(&generated, &aliases);
        assert!(script.starts_with("#compdef crabcode cc\n"));
        assert!(script.contains("compdef _crabcode 'crabcode' 'cc'"));
        assert!(!script.contains(r#"__usage_input="${__usage_input}-""#));
    }
